...
// Two outputs: the standalone SVG for the website, and a TS module embedding the
// same markup so the Worker can serve it live at /map.svg without a build step
// or a wrangler asset rule.
writeFileSync(ROOT + "docs/map.svg", svg);
writeFileSync(
  ROOT + "src/map-svg.ts",
  `// GENERATED by scripts/render-map.mjs (run \`npm run map\`). Do not edit by hand.\n` +
    `// The world map SVG, embedded so the Worker can serve it at /map.svg.\n` +
    `export const MAP_SVG = ${JSON.stringify(svg)};\n`,
);
console.error(`wrote docs/map.svg + src/map-svg.ts (${svg.length} bytes, ${Object.keys(ROOMS).length} rooms)`);
